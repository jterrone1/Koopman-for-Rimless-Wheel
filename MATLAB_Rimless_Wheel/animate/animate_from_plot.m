% plots animation for full spoke model (such as state output data)

addpath("rimless_scripts\")
filename = ".\..\..\Data_Rimless_Wheel\sim_results\z_out\z_out_31_traj_CCK_B_p20.csv";
...
Il = 8.47 * 10^-5;
% assuming a steel lead screw with diameter 46 mm and length 175 mm,
% mass = rho * volume = 0.726 kg, inertia = 1/2 m r^2 = 8.47 * 10^-5
% density of steel is 7850 kg/m3

Im = 1.15*10^-3;
% assuming a steel motor with diameter 60 mm and length 120 mm, mass = rho * volume = 2.55 kg, inertia = 1/2 m r^2 = 1.15*10^-3
% the motor housing is approximately 100 mm x 200 mm

ramp = 0 * (pi/180);
p   = [M m I l g ramp k l0 r R Il Im]';       % parameters

range_x = [min(z_out(1,:))-3*l,max(z_out(1,:))+l];
...
myVideo = VideoWriter('test', 'MPEG-4');
...
% Adjust tick labels and general font
ax = gca;
ax.FontSize = 8; 
ax.FontWeight = 'bold';
ax.LineWidth = 1; 
...
for i=1:length(z_out)
    ...
    set(h_title,'String',  sprintf('CCK, N = 20, t = %.2f',t) ); % update title
