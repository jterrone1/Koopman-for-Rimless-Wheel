...
def RBF(center, x, eps, kinds="gauss"):
    ...
    """
    c = center
    epsilon = eps # This is the hyperprameter!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    # 0.3 for 100 mm
...
def edmdc(X, Y, U, centers, b_val, dt, lam, eps):
    X_lift = X[1:12, :].T
    Y_lift = Y[1:12, :].T
    for c in centers:
    ...
    X_lift = X_lift.T
    Y_lift = Y_lift.T
    Omega = np.vstack((X_lift, U))

    A_lift = Y_lift@np.linalg.pinv(Omega)
    B_lift = A_lift[:,-3:]
    A_lift = A_lift[:, :-3]


    np.save(r"./../../Data_Rimless_Wheel/koopman_models/dmdc/A_test.npy", A_lift)
    np.save(r"./../../Data_Rimless_Wheel/koopman_models/dmdc/B_test.npy", B_lift)
    np.save(r"./../../Data_Rimless_Wheel/koopman_models/dmdc/centers_test.npy", centers)
...
def error_1dt(csv_xt, csv_xtp1, u_bool, u, A, B, centers, dt, eps, norm_vals = None):
    ...
    for i in range(len(data_t)):
        ...
        #error += (LA.norm(Z_out[0:11] - data_tp1[i, 1:12]))**2
...
def auto_epsilon(X, Y, U, centers, dt, eps_linspace, comparisonX, comparisonY, comparisonU=None, norm_vals=None):
    ...
    for eps in eps_linspace:
        print(eps)
        A_lift, B_lift = edmdc(X, Y, U, centers, (Il * pow(R, 2) + Im), dt, 0, eps)  # alpha used for ridge regularization
...
if 1:
    # add in actuated data
    xt_act = pd.read_csv('./../../Data_Rimless_Wheel/training/X_t_2000_act_tb_LA36_0.01dt.csv', header=None)
    xt1_act = pd.read_csv('./../../Data_Rimless_Wheel/training/X_tp1_2000_act_tb_LA36_0.01dt.csv', header=None)
    u_act = pd.read_csv('./../../Data_Rimless_Wheel/training/inputs_2000_act_tb_LA36_0.01dt.csv', header=None)

    X_true_act = xt_act.values.T
    Y_true_act = xt1_act.values.T
    U_true = u_act.values.T

    U_true = np.hstack((0*U_true, U_true))
    X_true = np.hstack((X_true, X_true_act))
    Y_true = np.hstack((Y_true, Y_true_act))
...
[X, Y] = normalize_data(X_true, Y_true, norm_vals) # normalizes data wrt max and min of the Y dataset (expected to have a larger range)
U = U_true/(0.160/(R*r)) #normalize input

obs_dim = 100
lift_dim = state_dim + obs_dim
centers = kmeans_calc(X, obs_dim)

csv_xt = './../../data/rimless_wheel/LINAK/cck/comparison/X_t_50_pass_tb_LA36_0.01dt_comparison.csv'
csv_xtp1 = './../../data/rimless_wheel/LINAK/cck/comparison/X_tp1_50_pass_tb_LA36_0.01dt_comparison.csv'
csv_u = ''

#eps_linspace = np.linspace(10,70, 20)
#[best_eps, A_lift, B_lift] = auto_epsilon(X, Y, U, centers, dt, eps_linspace, csv_xt, csv_xtp1, norm_vals = norm_vals)

best_eps = 1.5
A_lift, B_lift = edmdc(X, Y, U, centers, dt/((Il*pow(R,2) + Im)*(0.160/(R*r))), dt, 0, best_eps) # lambda used for ridge regularization
# model is saved to Data_Rimless_Wheel/koopman_models/dmdc/